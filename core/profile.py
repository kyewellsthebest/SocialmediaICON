...
Guessing from a listing row is not enough for this. A channel's category can be
wrong or stale, a title can be in English on a stream that is not, and "is this
a gaming stream" is not answerable from the word "IRL". So this builds a small
dossier per channel and keeps it: what they actually do, what they are known
for, what language they broadcast in, and whether the thing on screen is a
game.

Three layers, cheapest first, and most channels never reach the third:

* **The listing row itself.** A category on the block list is a rejection and
  costs nothing. So is a title in a script the audience cannot read.
...
KEY = "clipengine:profile:{channel}"
...
#: Categories that are a rejection on sight, matched loosely because Kick's
#: category names are free text with regional variants. This is the cheap
#: layer: no HTTP, no model, no argument.
BLOCKED_CATEGORIES = (
    "battlegrounds", "pubg", "bgmi", "call of duty", "warzone", "fortnite",
    "valorant", "counter-strike", "cs2", "csgo", "league of legends", "dota",
    "apex legends", "overwatch", "rainbow six", "rocket league", "minecraft",
    "roblox", "gta", "grand theft auto", "fifa", "ea sports", "madden", "nba 2k",
    "rust", "escape from tarkov", "dead by daylight", "among us", "fall guys",
    "world of warcraft", "final fantasy", "elden ring", "chess", "hearthstone",
    "teamfight tactics", "clash", "free fire", "mobile legends", "e-sports",
    "esports", "speedrun", "retro", "simulator", "slots", "virtuals",
)
...
def blocked_category(category: str) -> str:
    """The blocked term this category matches, or empty."""
...
def from_listing(channel: str, *, category: str = "", title: str = "",
    ...
    """
    blocked = blocked_category(category)
    if blocked:
        return Profile(
            channel=channel, eligible=False, is_gaming=True, confidence=1.0,
            reason=f"{category or 'the category'} is a game",
            decided_by="category", language=language,
        )
...
camera. It does not want gameplay - a stream whose content is someone playing a
video game is out, even a very good one, and even if the streamer talks over it.
Watching a game and reacting to it on camera is still gameplay.

Two decisions and one description.

Is this stream in English? Not "does the streamer speak some English" - is the
broadcast in English. A Hindi-language esports broadcast with an English title
is not. If you are unsure, say so with a low confidence rather than guessing;
being wrong here wastes days of a watch slot.

Is the stream's content a video game? Just Chatting, IRL, podcasts, music,
sport, cooking, reaction content and gambling streams are not games. Anything
where the screen is mostly a game is.
...
SCHEMA = {
    ...
    "required": ["is_english", "is_gaming", "confidence", "about", "reason"],
    "properties": {
...
def research(channel: str, *, facts: dict[str, Any] | None = None) -> Profile:
    ...
    if gaming:
        eligible, why = False, f"gameplay stream - {reason}"
    elif not english:
...
def decide(
    ...
    *,
    ...
    # A game found on the channel record rather than in the listing row - a
    # stream that has just switched category, or one the directory mislabelled.
    for name in facts.get("categories") or []:
        blocked = blocked_category(name)
        if blocked:
            found = Profile(
                channel=channel, eligible=False, is_gaming=True, confidence=0.9,
                reason=f"they stream {name}", decided_by="kick categories",
            )
            remember(found)
            return found
